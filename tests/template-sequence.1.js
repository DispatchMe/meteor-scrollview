...
Tinytest.addAsync('dispatch:template-sequence - Test sequence offset 2', function(test, complete) {
  ...
  test.equal(container.innerHTML, '-2-1012', 'Rendered index did not match');
  ...
  test.equal(destroyed, 0);
  test.equal(container.innerHTML, '-2-101234', 'Rendered index did not match');
  ...
  test.equal(container.innerHTML, '-2-10', 'Rendered index did not match');
  ...
  test.equal(destroyed, 4);
  test.equal(container.innerHTML, '-2-101234567', 'Rendered index did not match');
  ...
  Meteor.setTimeout(function() {
    ...
    test.equal(created, 14, 'Created counter dont match');
    test.equal(destroyed, 4);
    test.equal(container.innerHTML, '891011121314151617', 'Rendered index 10 did not match');
    ...
    Meteor.setTimeout(function() {
      ...
      test.equal(created, 14, 'Created counter dont match');
      test.equal(destroyed, 4);
      test.equal(container.innerHTML, '-12-11-10-9-8-7-6-5-4-3', 'Rendered index 10 did not match');
